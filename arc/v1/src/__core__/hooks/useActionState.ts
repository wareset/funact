...
interface HookDataForUseActionState extends HookData {
  valueTemp: any
  action: any
  pending: boolean
  queue: [any, any][]
...
function useActionState<State, Payload>(
  ...
  const hooks = vNode.hooks as HookDataForUseActionState[]
  const data =
    hooks[hookIdx] ||
    (hooks[hookIdx] = {
      hookIdx,
    ...
    } satisfies HookDataForUseActionState)
  checkHook(data, useActionState, hookIdx, vNode)
